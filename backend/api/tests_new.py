...
class ReviewAndPromoTestCase(TestCase):
    ...
    def test_review_correction_flow(self):
        ...
        # Wait, `offer_refund` should be done by Producer. My ViewSet logic didn't check user is owner of producer.
    ...
    def test_create_review_with_photo(self):
        ...
        image_content = b'\x47\x49\x46\x38\x39\x61\x01\x00\x80\x00\x00\xff\xff\xff!\xf9\x04\x01\x00;'
...
class ProfileUpdateTestCase(TestCase):
    ...
    def test_buyer_profile_update(self):
        ...
        self.assertIsNone(response.data.get('shop_name'))
    ...
    def test_shop_description_ai_generate_and_sanitize(self):
        self.client.force_authenticate(user=self.seller)
        # Create Seller
        self.seller_user = User.objects.create_user(username='seller@test.com', email='seller@test.com', password='password123')
        self.producer = Producer.objects.create(user=self.seller_user, name='New Shop Name', city='City')
        ...
        response = self.client.patch('/api/auth/me/', {
            ...
            'address': 'Address',
            'short_description': 'Short',
            'description': 'Tasty food made with love',
        })

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.producer.refresh_from_db()
        self.assertEqual(self.producer.name, 'New Shop Name')
        self.assertEqual(self.producer.city, 'City')
        self.assertEqual(self.producer.address, 'Address')
        self.assertEqual(self.producer.short_description, 'Short')
        self.assertEqual(self.producer.description, 'Tasty food made with love')
