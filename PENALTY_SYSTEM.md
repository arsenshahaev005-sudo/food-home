...
Когда магазин отклоняет заказ **ИЛИ** не успевает принять заказ вовремя (истекает `acceptance_deadline`):
...
**Важно:** Автоотмена по истечению времени принятия (`acceptance_deadline`) считается отказом продавца и применяются те же санкции, что и при ручном отклонении.
...
### Сценарий 4: Автоотмена по истечению времени (таймаут)
1. Cron-задача запускает команду `python manage.py process_order_timeouts` каждые 5 минут
2. Команда находит все заказы в статусе `WAITING_FOR_ACCEPTANCE` с истёкшим `acceptance_deadline`
3. Для каждого просроченного заказа вызывается `OrderService.reject_order()` с `apply_penalty=True`
4. Применяются те же санкции, что и при ручном отклонении:
   - +1 штрафное очко
   - +1 к `consecutive_rejections`
   - Штраф 30% от стоимости товара
   - Автоматический отзыв с 1 звездой
   - Возможная блокировка при 3+ отказах подряд
5. `cancelled_by` устанавливается как `"SELLER"` (так как продавец не успел принять заказ)
6. Покупателю возвращаются деньги

**Важно:** Автоотмена по таймауту = отказ продавца. Если продавец не успел принять заказ вовремя, это его ответственность.

### Сценарий 5: Разблокировка (поддержка)
...
1. **Автоотмена по таймауту = отказ продавца** - если магазин не успел принять заказ до истечения `acceptance_deadline`, применяются те же штрафы, что и при ручном отклонении
2. **Cron-задача для автоотмены**: необходимо настроить запуск `python manage.py process_order_timeouts` каждые 5 минут
3. **Штрафные очки не обнуляются при разблокировке** - магазин должен оплатить каждое очко отдельно
4. **Оплата штрафа НЕ разблокирует магазин** - нужно обращаться в поддержку
5. **При успешном принятии заказа** счетчик `consecutive_rejections` обнуляется
6. **Штраф применяется при любой отмене продавцом** (как до принятия, так и после)
7. **Баланс магазина должен быть достаточным** для оплаты штрафа
